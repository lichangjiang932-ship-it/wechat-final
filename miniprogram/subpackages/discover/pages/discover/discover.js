// subpackages/discover/pages/discover/discover.js - 发现页
...
Page({
  ...
  onImageTap(e) {
    const item = e.currentTarget.dataset.item;
    wx.navigateTo({
      url: `/subpackages/preview/pages/preview/preview?url=${encodeURIComponent(item.cover)}&title=${encodeURIComponent(item.title)}&likes=${item.likes}`
    });
  },
  ...
  onShareAppMessage() {
    return { title: '照片工坊 - 发现灵感', path: '/subpackages/discover/pages/discover/discover' };
  },
});
