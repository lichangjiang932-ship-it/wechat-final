...
const { computeNavBar, formatDate, isIOSPlatform } = require('../../utils/common');
  ...
  async loadUserInfo() {
    try {
      ...
      const [profile, usage] = await Promise.all([
        callFunction('user', { action: 'getInfo' }, { silent: true }).catch(() => null),
        callFunction('ai', { action: 'usage' }, { silent: true }).catch(() => null),
      ]);

      if (profile) {
        // 合并云端 profile 与本地 userInfo：云端字段（vipLevel/vipExpireTime 等）优先，
        // 但 nickName/avatarUrl 在云端为空时保留本地值。
        // 因为 confirmLogin 只把头像昵称存到本地、没同步到云 DB，云端拿到的通常是空串，
        // 直接覆盖会把用户登录态"清空"，看上去像被强制登出。
        const localUser = (user && typeof user === 'object') ? user : (wx.getStorageSync('userInfo') || {});
        const merged = {
          ...localUser,
          ...profile,
          nickName: profile.nickName || localUser.nickName || '',
          avatarUrl: profile.avatarUrl || localUser.avatarUrl || '',
        };
        wx.setStorageSync('userInfo', merged);
        this.setData({
          userInfo: merged,
          handle: this.deriveHandle(merged.nickName),
        });
      }
      ...
      if (usage) {
        this.setData({
          usageInfo: {
            isVip: !!usage.isVip,
            limit: Number.isFinite(usage.limit) ? usage.limit : 5,
