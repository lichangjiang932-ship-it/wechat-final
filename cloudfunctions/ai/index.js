...
// ============ 会员价格配置 ============
// price 单位：元（仅展示用）；priceFen：分（用于支付）；productId：米大师后台配置的道具ID
const VIP_PRICES = {
  month: { price: 19.9,  priceFen: 1990,  days: 30,  name: '月度会员', productId: 'vip_month' },
  year:  { price: 199.9, priceFen: 19990, days: 365, name: '年度会员', productId: 'vip_year'  },
};
...
// ============ 小程序虚拟支付（微信公众平台 · 道具直购）配置 ============
// 在「微信公众平台 → 小程序虚拟支付 → 基本配置」获取以下值：
// XPAY_OFFER_ID:   支付应用 ID（OfferID）
// XPAY_APPKEY:     沙箱/正式 AppKey
// XPAY_ENV:        0=正式, 1=沙箱（官方语义）
// XPAY_PAYSIG_URI: paySig 签名用的 URI。
//   ⚠️ 客户端 wx.requestVirtualPayment 场景下，官方文档明确规定 URI **固定填字符串
//   "requestVirtualPayment"**（不是 HTTP 路径，没有斜杠）。
//   服务器 API（query_user_balance 等）才是用 /xpay/xxx 形式的路径。
//   见: https://developers.weixin.qq.com/miniprogram/dev/platform-capabilities/business-capabilities/virtual-payment.html
const XPAY_OFFER_ID = process.env.XPAY_OFFER_ID || process.env.MIDAS_OFFER_ID || '';
// Official semantics: env=0 production, env=1 sandbox.
const XPAY_ENV = String(process.env.XPAY_ENV || process.env.MIDAS_ENV || '0') === '1' ? 1 : 0;
// Recommended env vars:
// XPAY_APPKEY_PROD, XPAY_APPKEY_SANDBOX, XPAY_PAYSIG_URI
...
const XPAY_PAYSIG_URI = process.env.XPAY_PAYSIG_URI || process.env.XPAY_SIGN_PATH || 'requestVirtualPayment';

function getXpayAppKeyByEnv(env) {
  if (env === 1) return XPAY_APPKEY_SANDBOX || XPAY_APPKEY_FALLBACK;
...
const SECURITY_FAIL_OPEN = String(process.env.SECURITY_FAIL_OPEN || 'false').toLowerCase() === 'true';
  ...
  try {
    switch (action) {
      ...
      case 'img2img': // 图生图
        ...
        if (event.prompt && event.prompt.length > MAX_PROMPT_LENGTH) return { code: -1, msg: `描述不能超过${MAX_PROMPT_LENGTH}字` };
        if (!event.imageFileID) return { code: -1, msg: '请先上传参考图' };
        if (event.prompt) {
          const sec = await msgSecCheck(event.prompt, openid, requestId);
          if (!sec.pass) return { code: -1, msg: sec.msg || '内容含违规信息，请修改后重试' };
        }
        {
          const secImg = await imgSecCheckByFileID(event.imageFileID, openid, requestId);
          if (!secImg.pass) return { code: -1, msg: secImg.msg || '参考图含违规信息，请更换后重试' };
        }
      ...
      case 'createOrder': {
        ...
        // iOS 走小程序虚拟支付（米大师道具直购）；Android 走原微信支付
        if (event.platform === 'ios') {
          return await createVirtualOrder(openid, event.plan, requestId, event);
        }
        return await createOrder(openid, event.plan, requestId, event);
      }

      case 'createVirtualOrder': {
        // 显式调用虚拟支付（兼容客户端直接走 iOS 路径）
        const planInfo = validatePlan(event.plan);
        if (!planInfo.valid) return { code: -1, msg: planInfo.msg };
        return await createVirtualOrder(openid, event.plan, requestId, event);
      }

      case 'deliverVirtualOrder': {
        // 客户端在 wx.requestVirtualPayment success 回调里主动调用，
        // 把已支付订单"发货"（开通会员）。不依赖微信的 pay-notify-virtual 回调。
        return await deliverVirtualOrder(openid, event.outTradeNo, requestId);
      }
...
// ============================================================
// 小程序虚拟支付（微信公众平台 · 道具直购模式 short_series_goods）
// 客户端调用 wx.requestVirtualPayment({ signData, paySig, signature, mode })
// 参考：微信公众平台 → 小程序虚拟支付 → 开发指引
// ============================================================
function hmacSha256Hex(message, key) {
  return crypto.createHmac('sha256', key).update(message, 'utf8').digest('hex');
}

async function createVirtualOrder(openid, plan, requestId, event = {}) {
  const appKey = getXpayAppKeyByEnv(XPAY_ENV);
  if (!XPAY_OFFER_ID || !appKey) {
    log.error(requestId, '虚拟支付未配置 OFFER_ID/APPKEY');
    return { code: -1, msg: '虚拟支付服务未配置' };
  }
  if (!IS_TEST_MODE && XPAY_ENV !== 0) {
    log.error(requestId, 'production build blocked: XPAY_ENV must be 0');
    return { code: -1, msg: 'Invalid payment env: production requires env=0' };
  }
  if ((event.platform || '').toLowerCase() === 'ios' && XPAY_ENV === 1) {
    log.error(requestId, 'ios does not support sandbox env=1');
    return { code: -1, msg: 'iOS only supports production virtual payment env=0' };
  }

  const planInfo = VIP_PRICES[plan];
  if (!planInfo || !planInfo.productId) {
    return { code: -1, msg: '不支持的套餐' };
  }

  const outTradeNo = `xvip${Date.now()}${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`;

  // signData：道具直购模式
  // platform 由客户端在 wx.requestVirtualPayment 时自动注入，服务端不要写
  const signDataObj = {
    offerId: XPAY_OFFER_ID,
    buyQuantity: 1,
    env: XPAY_ENV,
    currencyType: 'CNY',
    productId: planInfo.productId,
    goodsPrice: planInfo.priceFen,
    outTradeNo,
  };
  // signData 必须是 JSON 字符串
  const signData = JSON.stringify(signDataObj);

  // paySig = HMAC-SHA256(请求路径 + '&' + signData, AppKey)
  const paySig = hmacSha256Hex(`${XPAY_PAYSIG_URI}&${signData}`, appKey);

  // signature = HMAC-SHA256(signData, sessionKey)
  // sessionKey 由 wx.login 的 code 通过 jscode2session 换取；客户端把 code 透传过来
  // 注意：cloud.openapi.auth.code2Session 出于安全性原因不返回 session_key，
  // 所以这里必须直接 HTTPS 请求 api.weixin.qq.com，用 AppID + AppSecret 换。
  // AppSecret 从云函数环境变量 WX_APP_SECRET 读取（在云开发控制台配置）。
  let signature = '';
  if (!event.code) {
    log.error(requestId, '客户端未传 wx.login 的 code');
    return { code: -1, msg: '登录态缺失，请重新登录后再试' };
  }
  const wxContext = cloud.getWXContext();
  const appId = wxContext.APPID;
  const appSecret = process.env.WX_APP_SECRET || '';
  if (!appSecret) {
    log.error(requestId, '缺少 WX_APP_SECRET 环境变量');
    return { code: -1, msg: '虚拟支付未配置：缺少 AppSecret' };
  }
  try {
    ...
    const data = sessionRes && sessionRes.data;
    const sessionKey = data && data.session_key;
    if (sessionKey) {
      signature = hmacSha256Hex(signData, sessionKey);
    } else {
      log.error(requestId, 'jscode2session 未返回 session_key', data);
      const errmsg = (data && data.errmsg) || '';
      const errcode = (data && data.errcode) || '';
      return { code: -1, msg: `登录态换取失败(${errcode}): ${errmsg || 'no session_key'}` };
    }
  } catch (e) {
    log.error(requestId, 'jscode2session 请求失败:', e.message);
    return { code: -1, msg: '登录态换取失败，请稍后重试' };
  }

  try {
    await safeAdd('virtual_orders', {
      openid,
      plan,
      planName: planInfo.name,
      productId: planInfo.productId,
      priceFen: planInfo.priceFen,
      price: planInfo.price,
      offerId: XPAY_OFFER_ID,
      env: XPAY_ENV,
      outTradeNo,
    ...
    log.error(requestId, '虚拟订单落库失败', e.message);
    return { code: -1, msg: '订单创建失败，请稍后重试' };
  }

  // 诊断信息（不打印完整密钥，只打首尾各 4 个字符 + 长度，便于排查 AppKey 配错）
  const appKeyFingerprint = appKey
    ? `${appKey.slice(0, 4)}***${appKey.slice(-4)}(len=${appKey.length})`
    : 'EMPTY';
  log.info(requestId, '虚拟订单已创建', {
    outTradeNo,
    plan,
    env: XPAY_ENV,
    paySigUri: XPAY_PAYSIG_URI,
    paySigUriBytes: Buffer.byteLength(XPAY_PAYSIG_URI, 'utf8'),
    appKeyFingerprint,
    signData,                          // 完整 signData，方便和客户端实际发的对照
    paySigPreview: paySig.slice(0, 16),
...
// ============================================================
// 客户端主动发货：wx.requestVirtualPayment success 后由客户端调用
// 不依赖微信平台的发货回调（消息推送在某些 mp 后台版本配置不通），
// 直接把已支付订单从 pending → success 并开通会员。
// ============================================================
async function deliverVirtualOrder(openid, outTradeNo, requestId) {
  if (!openid) return { code: -1, msg: '用户身份验证失败，请重新登录' };
  ...
  try {
    // 1) 查订单：必须存在 + 必须属于当前 openid（防止帮别人发货）
    const orderRes = await db.collection('virtual_orders')
      .where({ outTradeNo, openid })
      .limit(1)
      .get();
    order = orderRes.data && orderRes.data[0];
    if (!order) {
      log.warn(requestId, 'deliverVirtualOrder: order not found', { outTradeNo, openid });
      return { code: -1, msg: '订单不存在' };
    }

    // 2) 已发货：幂等返回成功
    if (order.status === 'success') {
      log.info(requestId, 'deliverVirtualOrder: already success', { outTradeNo });
      return {
        code: 0,
        data: {
          delivered: true,
          vipLevel: order.plan,
          vipExpireTime: order.membershipExpireTime || null,
        },
      };
    }

    // 3) 原子锁定：只有当 status=pending 才能 update 成 processing
    const now = Date.now();
    const lockRes = await db.collection('virtual_orders')
      .where({ _id: order._id, status: 'pending' })
      .update({
        data: {
          status: 'processing',
          clientDeliveredAt: now,
          deliveredBy: 'client',
          updateTime: now,
        },
      });
    ...
    if (!lockRes || !lockRes.stats || lockRes.stats.updated === 0) {
      // 别的并发请求/回调拿走了；重新读取一次
      const latestRes = await db.collection('virtual_orders').doc(order._id).get();
      ...
      if (latest && latest.status === 'success') {
        return {
          ...
          data: {
            delivered: true,
            vipLevel: latest.plan,
            vipExpireTime: latest.membershipExpireTime || null,
          },
        };
      }
      log.warn(requestId, 'deliverVirtualOrder: lock failed', { outTradeNo, currentStatus: latest && latest.status });
      return { code: -1, msg: '订单状态异常，请稍后重试' };
    }

    // 4) 计算新会员到期时间
    const planInfo = VIP_PRICES[order.plan];
    if (!planInfo) {
      // 数据脏了；回滚状态
      await db.collection('virtual_orders').doc(order._id).update({
        data: { status: 'failed', failReason: 'INVALID_PLAN', updateTime: Date.now() },
      });
      return { code: -1, msg: '订单套餐异常' };
    }
    ...
    const base = currentExpire > Date.now() ? currentExpire : Date.now();
    ...
    log.info(requestId, 'deliverVirtualOrder: success', {
      outTradeNo,
      openid,
      plan: order.plan,
      newExpire,
    ...
    log.error(requestId, 'deliverVirtualOrder: failed', { outTradeNo, message: e.message, stack: e.stack });
    // 统一回滚：尽可能把 processing 改回 pending，让回调/补偿任务有机会重试
    try {
      if (order && order._id) {
        const latestRes = await db.collection('virtual_orders').doc(order._id).get();
        const latest = latestRes && latestRes.data;
        if (latest && latest.status === 'processing') {
          await db.collection('virtual_orders').doc(order._id).update({
            data: {
              status: 'pending',
              failReason: `DELIVER_${e.message || 'ERR'}`,
              updateTime: Date.now(),
            },
          });
        }
...
async function msgSecCheck(content, openid, requestId) {
  ...
  try {
    ...
    if (SECURITY_FAIL_OPEN) return { pass: true };
    return { pass: false, msg: '内容审核服务暂不可用，请稍后重试' };
  } catch (e) {
...
// ============================================================
// 图片安全审核（微信 security.imgSecCheck）
// ============================================================
async function imgSecCheckByFileID(fileID, openid, requestId) {
  if (!fileID || typeof fileID !== 'string') {
    return { pass: false, msg: '参考图无效，请重新上传' };
  }
  try {
    const tempRes = await cloud.getTempFileURL({ fileList: [fileID] });
    const mediaUrl = tempRes?.fileList?.[0]?.tempFileURL;
    if (!mediaUrl) {
      return { pass: false, msg: '参考图读取失败，请重新上传' };
    }
    ...
    if (res && res.errCode === 0) {
      const suggest = (res.result && res.result.suggest) || 'pass';
      if (suggest === 'pass') return { pass: true };
      log.warn(requestId, '图片安全审核未通过', { suggest, label: res.result && res.result.label });
      return { pass: false, msg: '参考图含违规信息，请更换后重试' };
    }

    log.warn(requestId, '图片安全接口返回异常', res);
    if (SECURITY_FAIL_OPEN) return { pass: true };
    return { pass: false, msg: '图片审核服务暂不可用，请稍后重试' };
  } catch (e) {
    log.error(requestId, 'imgSecCheck 异常:', e.message);
    if (SECURITY_FAIL_OPEN) return { pass: true };
    return { pass: false, msg: '图片审核服务暂不可用，请稍后重试' };
  }
}
