...
async function lockPendingOrder(outTradeNo, callbackRaw) {
  try {
    ...
    return (lockRes && lockRes.stats && lockRes.stats.updated > 0) || false;
  } catch (e) {
    if (e && e.errCode === -502005) return false; // 集合未创建
    throw e;
  }
}
...
exports.main = async (event) => {
  ...
  try {
    ...
    const locked = await lockPendingOrder(outTradeNo, parsed);
    if (!locked) {
      const latestOrder = await getOrderByOutTradeNo(outTradeNo);
      if (latestOrder && latestOrder.status === 'success') {
        logInfo(requestId, '并发回调已由其他请求处理成功', { outTradeNo });
        return responseXml('SUCCESS', 'OK');
      }
      logInfo(requestId, '订单不在可处理状态，忽略重复处理', { outTradeNo, status: latestOrder && latestOrder.status });
      return responseXml('SUCCESS', 'OK');
    }
    ...
    if (event && event.body) {
      try {
        const parsed = parseXmlToJson(extractXmlBody(event));
        const outTradeNo = parsed && parsed.out_trade_no;
        if (outTradeNo) {
          await db.collection('orders').where({ outTradeNo, status: 'processing' }).update({
            data: {
              status: 'pending',
              updateTime: Date.now(),
            },
